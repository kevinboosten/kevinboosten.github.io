...
1. Create a file per environment and rebuild the complete application per environment. (e.g. `environment.ts`, `environment.tst.ts`, `environment.stg.ts`, `environment.prd.ts`).
...
Push this repo to your Github or Azure DevOps account so you can use it Azure DevOps.
...
Now that we have an Angular app and our project set up, we can configure our pipeline. You can do this via the portal or simply create a file `azure-pipelines.yml` in the root of your repository and and this as your starter pipeline to the yml file:
...
Multi-stage pipelines are still in preview, so it's possible that you have to toggle the preview feature. But at the time of writing I do not see the feature toggle anymore, so maybe it's in preview anymore 🎉.
...
Not much changes, just following the documentation from Microsoft. Thing that you will notice is the _environment_ setting. This is, in our case, just a label to group our deployment. You can now see your deployment stage in the portal too:
...
```typescript
title = `is production: ${environment.production}, auth: ${environment.auth}, api: ${environment.apiEndpoint}`;
```
...
Because we're not deploying our code to a working environment, we can't verify if the replacement action actually worked...What we can do is check the logging, so open the last run in the DevOps portal and click on the replacetokens job:
...
### Bonus: add dependencies and conditions
